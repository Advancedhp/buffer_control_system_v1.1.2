...
def sidebar_controls() -> None:
    ...
    with st.sidebar.expander("Пороговые значения статусов", expanded=False):
        ...
        if not (critical < normal < surplus):
            st.error("Пороги должны выполняться как: критический < норма < избыточность.")
    ...
    st.sidebar.metric("Средний Ibuf", metrics["avg_ibuf"])
...
def dashboard_tab(df: pd.DataFrame) -> None:
    ...
    c5.metric("Средний Ibuf", metrics["avg_ibuf"])
    ...
    st.info(
        f"Пороговые значения: критическое отклонение ≤ {st.session_state.critical_threshold:.2f}; "
...
def scenario_tab() -> None:
    ...
    metric = st.selectbox("Показатель для диаграммы изменения", ["Bopt", "Ibuf", "A", "Pshort_current", "reorder_point"])
    ...
    c1.metric("Средний Bopt", f"{before['Bopt'].mean():.2f}", delta=f"{after['Bopt'].mean() - before['Bopt'].mean():.2f}")
    c2.metric("Средний Ibuf", f"{before['Ibuf'].mean():.2f}", delta=f"{after['Ibuf'].mean() - before['Ibuf'].mean():.2f}")
...
def testing_tab() -> None:
    ...
    metric = st.selectbox("Показатель на графике", definition["metrics"], index=definition["metrics"].index(definition["main"]))
    ...
    if before.empty or after.empty:
        ...
        return
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Средний Bopt до", f"{before['Bopt'].mean():.2f}")
    c2.metric("Средний Bopt после", f"{after['Bopt'].mean():.2f}", delta=f"{after['Bopt'].mean()-before['Bopt'].mean():.2f}")
    c3.metric("Средний Ibuf до", f"{before['Ibuf'].mean():.2f}")
    c4.metric("Средний Ibuf после", f"{after['Ibuf'].mean():.2f}", delta=f"{after['Ibuf'].mean()-before['Ibuf'].mean():.2f}")
    left_chart, right_chart = st.columns(2)
    with left_chart:
        st.plotly_chart(
            metric_bar(before, metric=metric, title=f"{metric} до теста"),
            use_container_width=True,
            key=f"testing_before_chart_{selected_test}_{metric}",
        )
    with right_chart:
        st.plotly_chart(
            metric_bar(after, metric=metric, title=f"{metric} после теста"),
            use_container_width=True,
            key=f"testing_after_chart_{selected_test}_{metric}",
        )
    st.plotly_chart(
        scenario_delta_chart(before, after, metric=metric, title=f"Изменение {metric}: {selected_test}"),
        use_container_width=True,
...
- `rop_load` — загрузка ресурса, ограничивающего производство.
- `planned_flow`, `actual_flow` — плановый и фактический поток к ROP.
...
- `Kcrit` — коэффициент критичности позиции для ROP.
- `Kload` — коэффициент текущей загрузки ROP.
- `Kdev` — коэффициент отклонения фактического потока от планового.

В режиме `auto` коэффициенты рассчитываются автоматически. В режиме `manual` используются значения `Kcrit_manual`, `Kload_manual`, `Kdev_manual`.
