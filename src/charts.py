...
def pshort_bar(df: pd.DataFrame):
    ...
    data = _clean_numeric(df, [risk_col]).sort_values(risk_col, ascending=False).head(15)
...
def metric_bar(df: pd.DataFrame, metric: str = "Bopt", title: str | None = None, top_n: int = 15):
    """Bar chart for a single metric, used for before/after model tests."""
...
def scenario_delta_chart(before: pd.DataFrame, after: pd.DataFrame, metric: str = "Bopt", title: str | None = None):
    ...
    data = data.sort_values("abs_delta", ascending=False).head(15)
    fig = go.Figure()
